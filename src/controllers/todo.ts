...
// get all todo
export async function allTodo(req: CustomRequest, res: Response): Promise<void> {
    try {
        const result = await service.todoService.allTodo(req.user.data._id);
        if (!result) {
            responseData({ res, statusCode: statusCode.BADREQUEST, success: 0, message: responseMessage(error, getting,todo) });
            return;
        }
        responseData({ res, ...result });
    } catch (error:any) {
...
//  Single todo details
export async function singleTodo(req: CustomRequest, res: Response): Promise<void> {
    try {
        const result = await service.todoService.getSingleTodo(req.params.id,req.user.data._id);
        if (!result) {
            responseData({ res, statusCode: statusCode.BADREQUEST, success: 0, message: responseMessage(error, getting,todo) });
            return;
        }
